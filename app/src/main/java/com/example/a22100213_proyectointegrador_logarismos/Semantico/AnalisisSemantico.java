...
public class AnalisisSemantico {
    ...
    private static void validarSistemaGrado(ResultadoSemantico rs, NodoAST r) {
        ...
        for (NodoAST e : eqs) {
            ...
            for (String v : vs) {
                int gL = gradoEn(sub(e, 0), v);
                int gR = gradoEn(sub(e, 1), v);
                if (gL > 1 || gR > 1 || gL == Integer.MAX_VALUE || gR == Integer.MAX_VALUE) rs.errores.add(SemanticoError.error("T8_GRADO", "Todas de primer grado", ruta(e)));
            }
            if (!esLinealMultivariable(sub(e, 0), vs) || !esLinealMultivariable(sub(e, 1), vs)) rs.errores.add(SemanticoError.error("T8_LINEAL", "Sólo términos lineales", ruta(e)));
        }
    }
    ...
    private static boolean soloArit(NodoAST n) {
        ...
        if (t == LexToken.Type.SUM || t == LexToken.Type.SUB || t == LexToken.Type.MUL || t == LexToken.Type.DIV || t == LexToken.Type.EXP || t == LexToken.Type.PAREN_OPEN || t == LexToken.Type.PAREN_CLOSE) {
    ...
    private static void validarDominiosGenerales(ResultadoSemantico rs, NodoAST n) {
        ...
        if ((t == LexToken.Type.LN || t == LexToken.Type.LOG || t == LexToken.Type.LOG_BASE2 || t == LexToken.Type.LOG_BASE10) && n.hijos.size() == 1) {
            Double a = evalConst(sub(n, 0));
            if (a != null && a <= 0) rs.errores.add(SemanticoError.error("LOG_DOM", "Logaritmo con argumento ≤ 0", ruta(n)));
        }
        ...
        if ((t == LexToken.Type.TRIG_TAN || t == LexToken.Type.TRIG_COT) && n.hijos.size() == 1) {
            ...
            if (a != null) {
                double c = Math.cos(a);
                double s = Math.sin(a);
                if (t == LexToken.Type.TRIG_TAN && Math.abs(c) < 1e-15) rs.errores.add(SemanticoError.error("TAN_UNDEF", "tan indefinida para ese argumento", ruta(n)));
                if (t == LexToken.Type.TRIG_COT && Math.abs(s) < 1e-15) rs.errores.add(SemanticoError.error("COT_UNDEF", "cot indefinida para ese argumento", ruta(n)));
            }
        }
    ...
    private static void validarSistema(ResultadoSemantico rs, NodoAST r) {
        ...
        Set<String> vs = allVarsInEqs(eqs);
        if (vs.size() != eqs.size()) rs.errores.add(SemanticoError.error("T8_VAR", "El número de variables debe ser igual al de ecuaciones", ruta(r)));
        ...
        for (NodoAST eq : eqs) {
            Set<String> veq = varsNoI(eq);
            if (!veq.containsAll(vs)) rs.errores.add(SemanticoError.error("T8_VAR_EC", "Cada ecuación debe incluir todas las variables", ruta(eq)));
            if (!(eq.token != null && eq.token.type == LexToken.Type.EQUAL)) rs.errores.add(SemanticoError.error("T8_EQ", "Cada fila debe ser una ecuación", ruta(eq)));
        }
    }
    ...
    private static Double evalConst(NodoAST n) {
        ...
        if (t == LexToken.Type.INTEGER || t == LexToken.Type.DECIMAL) {
            try {
                return Double.valueOf(n.token.value);
            } catch (Exception e) {
                return null;
            }
        }
        ...
        if (t == LexToken.Type.SUM || t == LexToken.Type.SUB || t == LexToken.Type.MUL || t == LexToken.Type.DIV || t == LexToken.Type.EXP) {
            ...
            switch (t) {
                ...
                case MUL:
                    return a * b;
                case DIV:
                    return Math.abs(b) < 1e-15 ? null : a / b;
                case EXP:
                    return Math.pow(a, b);
            }
        }
    ...
    private static boolean esPolinomicaEn(NodoAST n, String var) {
        if (n == null || n.token == null) return false;
        LexToken.Type t = n.token.type;
        if (t == LexToken.Type.INTEGER || t == LexToken.Type.DECIMAL || t == LexToken.Type.CONST_E || t == LexToken.Type.CONST_PI || (t == LexToken.Type.VARIABLE && var.equals(n.token.value) && n.hijos.isEmpty())) return true;
    ...
    private static boolean esPolinomica(NodoAST n) {
        if (n == null || n.token == null) return false;
        LexToken.Type t = n.token.type;
        if (t == LexToken.Type.INTEGER || t == LexToken.Type.DECIMAL || t == LexToken.Type.VARIABLE) return n.hijos.isEmpty();
        if (t == LexToken.Type.SUM || t == LexToken.Type.SUB || t == LexToken.Type.MUL) {
    ...
    private static Set<String> allVarsInEqs(List<NodoAST> eqs) {
        Set<String> s = new LinkedHashSet<>();
        for (NodoAST e : eqs) s.addAll(varsNoI(e));
        return s;
    }
    ...
    private static boolean esLinealMultivariable(NodoAST n, Set<String> vs) {
        ...
        if (t == LexToken.Type.MUL) {
            ...
            if (aVar) return esLinealMultivariable(sub(n, 0), vs) && esConstante(sub(n, 1), vs);
    ...
    private static boolean contieneAlgunoDe(NodoAST n, Set<String> vs) {
        if (n == null) return false;
        if (n.token != null && n.token.type == LexToken.Type.VARIABLE && n.hijos.isEmpty() && n.token.value != null && vs.contains(n.token.value)) return true;
